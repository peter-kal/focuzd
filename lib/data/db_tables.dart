...
class SettingsVariables extends Table {
  ...
  BoolColumn get roundPlanningByDefault => boolean().withDefault(Constant(true))();
}

class RoundVariable extends Table{
 IntColumn get id => integer().autoIncrement()();
 BoolColumn get completed => boolean().withDefault(const Constant(false)).nullable()();
 TextColumn get propableCause => text().nullable()(); // to be used when the round hasn't gone as planned
 ...
 DateTimeColumn get expFinishTime => dateTime().nullable()(); // in the round-centric approach that can also be called planedFinishTime
...
class MemorySessionVariable extends Table {
  IntColumn get id => integer().autoIncrement()();
  IntColumn get roundGoal => integer()(); // will be deprecated, keeping it just for testing
